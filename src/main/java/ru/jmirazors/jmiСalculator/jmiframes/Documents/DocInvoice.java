...
public class DocInvoice extends javax.swing.JInternalFrame implements DocumentImpl {
    ...
    public static ArrayList<InvoiceProduct> products = new ArrayList<>();
    ...
    public void repaintDocument() {
        this.setTitle(frameTitle + " ["+docInvoice.getStatus().getName()+"]");                    // Заголовок
        if (docInvoice.getId() != 0)
            jLabel2.setText(""+docInvoice.getId());                                               // Номер документа
        
        jDateChooser2.setDate(docInvoice.getIndate());                                            // Дата документа
        jTextField1.setText(docInvoice.getDepartment().getName());                                // Подразделение
        jLabel14.setText(docInvoice.getUsr().getName());                                          // Пользователь
        jFormattedTextField1.setText(String.valueOf(docInvoice.getDiscount()).replace(".", ",")); // Скидка
        jTextField2.setText(docInvoice.getDescr());                                               // Примечание
        jComboBox1.setSelectedItem(docInvoice.getStorage().getName());                            // Склад
        jComboBox2.setSelectedItem(docInvoice.getPriceName().getName());                          // Тип цен
        jLabel6.setText(MainFrame.sessionParams.getParam().getOkv().getRus());                    // Валюта
        jLabel25.setText(MainFrame.sessionParams.getParam().getOkv().getRus());                   // Валюта
        jLabel16.setText(MainFrame.sessionParams.getParam().getOkv().getRus());                   // Валюта
        jLabel26.setText(docInvoice.getOrganization().getName());                                 // Организация
        
        if (docInvoice.getContragent() != null)
            jTextField3.setText(docInvoice.getContragent().getName());                            // Контрагент
        //recalculateDocument();
